...
def find_classes(ppmi, vocab, classes = set(),  max_k = 3, max_pcs = None):
    ...
    for i in range(pcas.shape[1]):
        ...
        clusters = [
            tuple(sorted([vocab_idx[x] for x in np.where(best_kmeans.labels_ == y)[0]])) for y in range(len(set(best_kmeans.labels_)))]
        for c in clusters:
            if c not in classes:
                classes.add(c)
                # print('\tnew class: ', c)
...
def cluster(pca_col, max_k):
    kmeans = None
    max_bic = -np.inf
    # best_k = compute_gap(KMeans(), pca_col, min(max_k, len(pca_col)))
    # print(best_k)
    kmeans = OPTICS(min_samples=2)
    best_kmeans = kmeans.fit(pca_col)
    # for k in range(1, min(pca_col.shape[0], max_k) + 1):
    #     kmeans = KMeans(n_clusters = k)
    #     clustering = kmeans.fit(pca_col)
    #     cl_bic = davies_bouldin_score(pca_col, clustering.labels_)
    #     print(cl_bic)
...
def compute_sil(clustering, data):
...
def compute_gap(clustering, data, k_max=5, n_references=5):
    ...
    print(np.log(reference_inertia), np.log(ondata_inertia))
    gap = 1 + np.argmax(np.log(reference_inertia)-np.log(ondata_inertia))
